...
)
{
    ...
    if (argc == 2 && !_strcmpi("--clear", argv[1])) {
        for (uint32_t i = 0; i < RTL_NUMBER_OF(EbpfTcSectionInfo); i++) {
            result = ebpf_store_delete_section_information(&EbpfTcSectionInfo[i]);
        ...
        result = ebpf_store_delete_program_information(&EbpfTcProgramInfo);
        if (result != EBPF_SUCCESS) {
        ...
        result = ebpf_store_update_section_information(&EbpfTcSectionInfo[0], RTL_NUMBER_OF(EbpfTcSectionInfo));
        if (result != EBPF_SUCCESS) {
            fprintf(stderr, "ebpf_store_update_section_information failed: %u\n", result);
            exit_code = -1;
        }

        result = ebpf_store_update_program_information(&EbpfTcProgramInfo, 1);
        if (result != EBPF_SUCCESS) {
